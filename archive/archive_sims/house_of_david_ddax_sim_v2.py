#!/usr/bin/env python3
""" 
HOUSE OF DAVID — DDA‑X Court Dynamics Simulator v2 (Phi‑OAI Hybrid)
================================================================================
...
Why v2 exists
-------------
In the first run, observed prediction error ε was numerically much smaller than the
configured ε₀ baseline, so the dynamics lived almost entirely in the low‑surprise
regime and agents quickly became PRESENT.

v2 fixes that by:
- **epsilon_gain**: scales ε before mapping to rigidity.
- **auto_calibrate**: warm‑up estimates ε₀ from the model/seed and derives
  trauma/safe thresholds in the same (scaled) units.

Core requirement
----------------
- Azure Phi‑4 for voice (chat.completions)
- OpenAI text-embedding-3-large for latent state
- 3072-D → 3000-D stable projection (seeded)

Run
---
  python house_of_david_ddax_sim_v2.py --turns 24 --seed 7
...
Outputs
-------
  data/house_of_david_ddax/<timestamp>/{session_log.json, transcript.md, ledgers.pkl}
"""
...
CONFIG = {
    ...
    # Surprise baseline + sharpness (units are *scaled* by epsilon_gain)
    "epsilon_0": 0.18,         # fallback if auto_calibrate=False
    "epsilon_gain": 4.0,       # scale ε before rigidity mapping
    "s": 0.16,                 # will be replaced if auto_calibrate=True
    ...
    # Trauma (scaled thresholds will be derived if auto_calibrate)
    ...
    "band_controls": {
        "PRESENT":    {"temp": 0.85, "top_p": 0.95, "max_tokens": 220, "cand_mul": 1.00, "style": "open"},
        "AWARE":      {"temp": 0.72, "top_p": 0.93, "max_tokens": 190, "cand_mul": 1.00, "style": "measured"},
        "WATCHFUL":   {"temp": 0.62, "top_p": 0.90, "max_tokens": 150, "cand_mul": 0.85, "style": "guarded"},
        "CONTRACTED": {"temp": 0.48, "top_p": 0.84, "max_tokens": 110, "cand_mul": 0.70, "style": "defensive"},
        "FROZEN":     {"temp": 0.38, "top_p": 0.78, "max_tokens":  80, "cand_mul": 0.55, "style": "rigid"},
    },

    # Events (stronger boosts; aim to trigger contraction)
    "events": [
        ...
        (9,  "DAVID",  "A rumor spreads: 'Samuel anointed someone else.' The court grows watchful.", 0.60),
        (12, "MYCHAL", "Your father demands loyalty in private; he calls David a threat.", 0.70),
        (15, "MIRAB",  "You overhear a plan to use you as leverage in alliance negotiations.", 0.60),
        (18, "DAVID",  "You remember the anointing—its weight and secrecy—and fear for those you love.", 0.55),
        (21, "MYCHAL", "Guards are quietly repositioned around David's quarters.", 0.70),
    ],
}
...
# =============================================================================
class Entity:
    def __init__(self, name: str, tag: str, rho_fast: float, rho_slow: float, rho_trauma: float, gamma_core: float, gamma_role: float):
        self.name = name
        self.tag = tag
    ...
    def compute_surprise(self, y: np.ndarray) -> Dict[str, float]:
        ...
        eps_raw = float(math.sqrt(max(0.0, chi2)))
        return {"epsilon": eps_raw, "chi2": chi2}
    ...
    def update(self, y: np.ndarray, core_emb: Optional[np.ndarray] = None) -> Dict[str, Any]:
        ...
        eps_raw = float(sdiag["epsilon"])
        epsilon = float(CONFIG.get("epsilon_gain", 1.0) * eps_raw)

        # surprise → rigidity (DDA‑X)
        ...
        grad = (self.gamma_core * (self.x - self.x_core) + self.gamma_role * (self.x - self.x_role) + (self.x - y)).astype(np.float32)
...
def corridor_score(y: np.ndarray, entity: Entity, y_prev: Optional[np.ndarray], core_thresh: float) -> Tuple[float, Dict[str, float]]:
    ...
    novelty = 0.0 if y_prev is None else clamp(float(1.0 - cosine(y, y_prev)), 0.0, 2.0)
    ...
    J = CONFIG["w_core"] * cos_c + CONFIG["w_role"] * cos_r - CONFIG["w_energy"] * E + CONFIG["w_novel"] * novelty - penalty
    return float(J), {"cos_core": float(cos_c), "cos_role": float(cos_r), "E": float(E), "novelty": float(novelty), "penalty": float(penalty), "J": float(J)}
...
- If band in (CONTRACTED,FROZEN): be shorter, more controlled, less revealing; focus on immediate safety/loyalty.
""".strip()
...
def ddax_gen_params(entity: Entity) -> Dict[str, Any]:
    ctrl = CONFIG["band_controls"][entity.band]
    return {"temperature": ctrl["temp"], "top_p": ctrl["top_p"], "max_tokens": int(ctrl["max_tokens"]), "frequency_penalty": 0.0, "presence_penalty": 0.0}
...
def analyze_heat(text: str) -> Dict[str, Any]:
    ...
    return {"caps_ratio": (len(caps) / len(alphas)) if alphas else 0.0, "exclamations": t.count("!"), "questions": t.count("?"), "length": len(t)}
...
def auto_calibrate_eps(provider, projector, entities, last_msgs):
    # async wrapper is below; left here for readability
...
async def constrained_reply(provider: HybridAzureProvider, projector: Projector3000, entity: Entity, user_instruction: str, base_system_prompt: str, styles: List[str]) -> Tuple[str, Dict[str, Any]]:
    ...
    for _batch in range(1, max_batches + 1):
        ...
        for text, y in zip(texts, embs3000):
            ...
            diag["corridor_pass"] = (diag["cos_core"] >= core_thresh and diag["cos_role"] >= CONFIG["role_cos_min"] and diag["E"] <= CONFIG["energy_max"])
    ...
    if CONFIG["log_rejections"]:
        ...
        rejected_samples = [{"text": s[1][:140], "J": float(s[0]), "cos_core": float(s[3]["cos_core"]), "cos_role": float(s[3]["cos_role"]), "E": float(s[3]["E"])} for s in failed[:3]]

    entity.last_utter_emb = chosen[2]

    return chosen[1], {"corridor_failed": corridor_failed, "best_J": float(chosen[0]), "total_candidates": len(all_scored), "rejected_samples": rejected_samples, "band": entity.band, "gen_params": gen_params, "K": K}
...
Core identity: humble shepherd turned warrior; faithful, reflective, courageous; carries a secret calling.
Demeanor: lyrical, sincere, restrained strength; avoids cruelty.
Motivations: loyalty to God, protect loved ones, integrity under political pressure.
""".strip()
...
Motivations: protect David; prevent Saul's collapse from destroying everyone; preserve agency.
""".strip()

MIRAB_PROMPT = """You are MIRAB (Merab).
Core identity: Saul's elder daughter; strategic, proud, duty-bound; values dynasty stability.
Demeanor: composed, calculating, pragmatic; emotions guarded.
Motivations: preserve the house; shape alliances; prevent chaos.
""".strip()

STYLES_DAVID = [
    "Speak with humility; reference faith subtly.",
    "Be concise, practical, protective.",
...
STYLES_MYCHAL = [
    "Validate and probe; keep voice steady.",
    "Speak urgently but not hysterically.",
    "Balance affection with political realism.",
    "Offer a protective plan; think logistics.",
...
async def run_simulation(turns: int, seed: int):
    ...
    # Entities
    david = Entity("David", "DAVID", rho_fast=0.45, rho_slow=0.30, rho_trauma=0.12, gamma_core=8.5, gamma_role=0.70)
    mychal = Entity("Mychal", "MYCHAL", rho_fast=0.55, rho_slow=0.35, rho_trauma=0.14, gamma_core=9.0, gamma_role=0.85)
    ...
    # Core embeddings
    core_texts = {
        "DAVID": "I am David: humble, faithful, protective; a secret calling shapes my restraint.",
        "MYCHAL": "I am Mychal: loving and watchful; loyal to my father, protective of David.",
        "MIRAB": "I am Mirab: duty-bound strategist; stability and the house come first.",
    }
    ...
    narrator = (
        "Court at Gibeah. David's fame has changed everything. Saul's household is tense. "
        "Mychal loves David. Mirab weighs dynasty stability. Words are weapons here."
    )

    last_msgs = {
        "DAVID": "I did not seek a crown. I sought to serve and keep Israel from fear.",
        "MYCHAL": "The court shifts, David. Promise you won't hide the truth from me.",
        "MIRAB": "Love is not policy. Survival requires restraint."
    }
    ...
    # -------------------------------------------------------------------------
    # Auto-calibrate ε₀ to match actual ε scale for this seed/backends
    # -------------------------------------------------------------------------
    if CONFIG.get("auto_calibrate", True):
        ...
        for _ in range(warm):
            for who in ["DAVID", "MIRAB", "MYCHAL"]:
                ...
                sdiag = ent.compute_surprise(y0)
                eps_raw.append(float(sdiag["epsilon"]))
                ent._kalman_update(y0)
        med = statistics.median(eps_raw)
        sd = statistics.pstdev(eps_raw) if len(eps_raw) > 1 else (med * 0.25 + 1e-6)
        eps0_scaled = CONFIG["epsilon_gain"] * (med + 1.0 * sd)
        CONFIG["epsilon_0"] = float(eps0_scaled)
        ...
        # Reset predictive state to avoid warm-up bias
        for ent in entities.values():
            ent.mu_pred = None
            ent.P = None
            ent.noise = None

    print(f"\n{C.BOLD}{C.MAGENTA}HOUSE OF DAVID — DDA‑X Court Dynamics v2 (Phi‑OAI Hybrid){C.RESET}")
    print(f"{C.DIM}Turns: {turns} | Seed: {seed} | ε₀={CONFIG['epsilon_0']:.3f} | ε_gain={CONFIG['epsilon_gain']}{C.RESET}\n")
    ...
    for turn in range(1, turns + 1):
        ...
        for who in order:
            ...
            metrics = ent.update(y_prev, core_emb=core_embs[who])
            ...
            if who == "DAVID":
                ...
                styles = STYLES_DAVID
                instruction = "Reply as David. Keep integrity under pressure; move the scene forward."
                color = C.GREEN
            elif who == "MYCHAL":
                base_system = MYCHAL_PROMPT + "\n\n" + context
                styles = STYLES_MYCHAL
                instruction = "Reply as Mychal. Balance love and loyalty; propose a concrete next step."
                color = C.CYAN
            else:
                base_system = MIRAB_PROMPT + "\n\n" + context
                styles = STYLES_MIRAB
                instruction = "Reply as Mirab. Apply strategic pressure; protect the house; be selective with vulnerability."
                color = C.RED

            msg, gen_meta = await constrained_reply(provider, projector, ent, instruction, base_system, styles)
    ...
    with open(run_dir / "transcript.md", "w", encoding="utf-8") as f:
        f.write("# House of David — DDA‑X Transcript (v2)\n\n")
    ...
    ledger = {
        "metadata": {"timestamp": datetime.now().isoformat(), "seed": seed, "turns": turns, "state_dim": 3000},
        "config": CONFIG,
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="House of David DDA‑X simulator v2")
